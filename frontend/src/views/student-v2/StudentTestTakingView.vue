...
const handleCancel = async () => {
  try {
    await ElMessageBox.confirm('작성한 답안이 저장되지 않습니다. 취소하시겠습니까?', '시험 취소', {
...
<template>
  <div class="student-page student-page--wide test-taking-page" v-loading="loading">
    <header class="test-header">
      <button class="student-icon-button" aria-label="시험 나가기" @click="handleCancel">
      ...
      <article v-for="question in questions" :key="question.id" class="question-card">
        <div class="question-card__header">
          ...
          <div class="question-meta">
            ...
            <span
              >{{ question.points }}점<span v-if="question.textbookProblem?.topic">
                · {{ question.textbookProblem.topic }}</span
              ></span
            >
          </div>
          <button
        ...
        >
          <el-radio-button v-for="choice in 5" :key="choice" :label="String(choice)">{{
            choice
          }}</el-radio-button>
        </el-radio-group>
        ...
        <div v-else class="essay-guide">
          <span class="essay-guide__icon"
            ><el-icon><Memo /></el-icon
          ></span>
          <div>
            <strong>답안지에 직접 작성해 주세요</strong>
            <p>이 문제는 선생님이 확인한 뒤 점수가 반영됩니다.</p>
          </div>
        </div>
      </article>
    </main>
