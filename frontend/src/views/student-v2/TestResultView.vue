...
const typeLabel = (t?: string) =>
  ({ OBJECTIVE: '객관식', SUBJECTIVE: '주관식', ESSAY: '서술형' } as Record<string, string>)[t ?? ''] ?? ''

onMounted(fetch)
</script>

<template>
  <div v-loading="loading" :class="isStudent ? 'student-page student-page--wide result-page' : 'teacher-result-page'">
    <el-card class="result-summary" shadow="never" style="margin-bottom: 12px">
      <div style="display: flex; align-items: center; gap: 12px">
        <el-button class="result-back" aria-label="이전 화면으로" @click="goBack" :icon="ArrowLeft" />
...
<style scoped>
.teacher-result-page { max-width: 960px; margin: 0 auto; padding: 16px; }
.result-page { display: grid; gap: 16px; padding-bottom: 28px; }
.result-page .result-summary,
.result-page .result-topics,
.result-page .result-answers { margin: 0 !important; border: 1px solid var(--student-border); border-radius: 19px; box-shadow: var(--student-shadow-sm); }
.result-page .result-summary { overflow: hidden; border: 0; color: #fff; background: linear-gradient(135deg, var(--student-primary-strong), #5678e8); box-shadow: 0 16px 34px rgba(28, 68, 166, .2); }
.result-page .result-summary :deep(.el-card__body) { padding: 20px; }
.result-page .result-summary :deep(.el-button) { border: 1px solid rgba(255,255,255,.32); color: #fff; background: rgba(255,255,255,.12); }
.result-page .result-summary :deep(.el-tag) { border-color: rgba(255,255,255,.5); color: #fff; background: rgba(255,255,255,.12); }
.result-page .result-summary :deep(.el-card__body > div > div:nth-child(2) > div) { color: rgba(255,255,255,.72) !important; }
.result-page .result-summary :deep(.el-card__body > div > div:nth-child(2) > div:nth-child(2)) { color: #fff !important; font-size: 20px !important; font-weight: 800 !important; }
.result-page .result-summary :deep(.el-card__body > div > div:last-child div) { color: rgba(255,255,255,.78) !important; }
.result-page .result-summary :deep(.el-card__body > div > div:last-child div:nth-child(2)) { color: #fff !important; font-size: 30px !important; }
.result-page .result-topics :deep(.el-card__header),
.result-page .result-answers :deep(.el-card__header) { padding: 17px 18px; border-bottom-color: var(--student-border); color: var(--student-ink); font-size: 16px; }
.result-page .result-topics :deep(.el-card__body),
.result-page .result-answers :deep(.el-card__body) { padding: 16px; }
.result-page .result-topic-card { padding: 14px !important; border: 1px solid var(--student-border) !important; border-radius: 14px !important; background: var(--student-surface-soft) !important; }
.result-page .result-answer-card { padding: 16px !important; border: 1px solid var(--student-border) !important; border-left-width: 5px !important; border-radius: 15px !important; box-shadow: 0 3px 12px rgba(31, 45, 73, .04); }
.result-page .result-video-link { display: flex !important; min-height: 44px; align-items: center; justify-content: center; margin-top: 13px !important; border-radius: 11px !important; background: var(--student-primary-soft) !important; color: var(--student-primary) !important; font-weight: 750 !important; }
@media (max-width: 520px) {
  .result-page .result-summary :deep(.el-card__body) { padding: 17px; }
  .result-page .result-summary :deep(.el-card__body > div) { align-items: flex-start !important; gap: 10px !important; }
  .result-page .result-summary :deep(.el-card__body > div > div:last-child) { flex: 0 0 auto; }
}
</style>
