...
function openTest(test: StudentTest) {
  router.push(
    getSubmission(test.id) ? `/student/tests/${test.id}/result` : `/student/tests/${test.id}`,
  )
}
...
<template>
  <div class="student-page student-page--wide exams-page" v-loading="loading">
    <StudentPageHeader
      eyebrow="MY TESTS"
      title="시험"
      subtitle="응시할 시험과 지난 결과를 한곳에서 확인해요."
    >
      <template #action
        ><span class="exams-header-icon"
    ...
    <section class="exam-summary" aria-label="시험 현황">
      <button
        type="button"
        :class="{ 'is-active': activeFilter === 'untaken' }"
        @click="activeFilter = 'untaken'"
      >
        <span>미응시 시험</span>
        <strong>{{ untakenTests.length }}<small>개</small></strong>
        <em
          >미응시만 보기 <el-icon><ArrowRight /></el-icon
        ></em>
      </button>
      <button
        type="button"
      ...
      >
        ...
        <em
          >완료 결과 보기 <el-icon><ArrowRight /></el-icon
        ></em>
      </button>
    </section>
      ...
      <div v-if="filteredTests.length" class="student-surface student-list">
        ...
        >
          ...
          <span class="student-list-row__content">
            ...
            <small
              >{{ formatLessonDate(test.lessonDate)
              }}<span v-if="test.className"> · {{ test.className }}</span></small
            >
...
.exam-summary button {
  ...
  box-shadow: 0 7px 20px rgba(36, 87, 214, 0.09);
  font: inherit;
  text-align: left;
  cursor: pointer;
  transition:
...
.exam-summary button:hover {
  border-color: var(--student-primary);
  background: var(--student-surface-hover);
  box-shadow: 0 10px 24px rgba(36, 87, 214, 0.14);
  transform: translateY(-2px);
}
.exam-summary button:active {
  transform: translateY(0) scale(0.985);
}
.exam-summary button:focus-visible {
  outline: 3px solid rgba(36, 87, 214, 0.25);
  outline-offset: 3px;
}
.exam-summary button.is-active {
  border-color: var(--student-primary);
  background: var(--student-blue-50);
  box-shadow: 0 0 0 2px var(--student-primary-soft);
}
.exam-summary span {
...
.exam-filters button.is-active {
  color: var(--student-primary-strong);
  background: var(--student-surface);
  box-shadow: 0 2px 8px rgba(28, 46, 78, 0.08);
}
.exam-row__pending {
  width: fit-content;
  margin-top: 3px;
