...
public class RememberMeService {
    ...
    @Transactional
    public void issue(HttpServletRequest request,
                      HttpServletResponse response,
                      String userRole,
                      Long userId) {
        LocalDateTime now = LocalDateTime.now();
        revokeRequestToken(request, now);
        rememberMeTokenRepository.deleteExpiredOrRevoked(now);
        enforceActiveTokenLimit(userRole, userId, now);
    ...
    public void clear(HttpServletRequest request, HttpServletResponse response) {
        revokeRequestToken(request, LocalDateTime.now());
        clearCookie(response);
    }

    private void revokeRequestToken(HttpServletRequest request, LocalDateTime now) {
        String cookieValue = cookieValue(request);
        ...
        if (parts.length == 2) {
            rememberMeTokenRepository.findBySelectorHash(hash(parts[0]))
                    .ifPresent(token -> revoke(token, now));
        }
    }

    private void enforceActiveTokenLimit(String userRole, Long userId, LocalDateTime now) {
        List<RememberMeToken> activeTokens = rememberMeTokenRepository
                .findByUserRoleAndUserIdAndRevokedAtIsNullOrderByCreatedAtDesc(userRole, userId);
        int tokensToKeepBeforeIssue = MAX_ACTIVE_TOKENS_PER_USER - 1;
        for (int i = tokensToKeepBeforeIssue; i < activeTokens.size(); i++) {
            revoke(activeTokens.get(i), now);
        }
    }
