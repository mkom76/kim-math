...
@Filter(name = "ownerFilter",   condition = "lesson_video_id IN (SELECT lv.id FROM lesson_videos lv JOIN lessons l ON l.id = lv.lesson_id WHERE l.class_id IN (SELECT ac.id FROM academy_classes ac WHERE ac.owner_teacher_id = :teacherId UNION SELECT ca.class_id FROM class_assistants ca WHERE ca.teacher_id = :teacherId))")
@Entity
