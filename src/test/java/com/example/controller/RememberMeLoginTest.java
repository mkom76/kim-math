...
class RememberMeLoginTest {
    ...
    @Test
    void remember_me_limits_active_tokens_per_user() throws Exception {
        Academy academy = academyRepository.save(Academy.builder().name("토큰 제한 학원").build());
        AcademyClass clazz = academyClassRepository.save(AcademyClass.builder()
                .name("토큰 제한반")
                .academy(academy)
                .build());
        ...
        for (int i = 0; i < 7; i++) {
            mockMvc.perform(post("/api/auth/student/login")
                            .contentType(MediaType.APPLICATION_JSON)
                            .content("""
                                    {"studentId":%d,"pin":"2222","rememberMe":true}
                                    """.formatted(student.getId())))
                    .andExpect(status().isOk());
        }

        assertThat(rememberMeTokenRepository
                .findByUserRoleAndUserIdAndRevokedAtIsNullOrderByCreatedAtDesc("STUDENT", student.getId()))
                .hasSize(5);
    }
}
